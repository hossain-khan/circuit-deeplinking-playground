...
class ComposeAppComponentFactory : AppComponentFactory() {
  ...
  private inline fun <reified T : Any> getInstance(
    ...
    // Load the class using the provided ClassLoader
    val clazz = Class.forName(className, false, classLoader).asSubclass(T::class.java)
    val kotlinClass = clazz.kotlin
    val modelProvider = providers[kotlinClass] ?: return null
    return modelProvider.value as T
  }

  /**
   * Called by the Android system to instantiate an activity. This method checks if the
   * activity can be provided by the Metro component. If the Metro component can provide
  ...
  override fun instantiateApplicationCompat(
    ...
    activityProviders = (app as ComposeApp).appGraph().activityProviders
    return app
  }
